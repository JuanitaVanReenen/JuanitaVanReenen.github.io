...
function assertMessageInput(input={}){
  ...
  const type=input.type||'text';if(!TYPES.has(type))throw new Error('Unsupported message type');
  ...
  if(type==='text'&&!input.body.trim()&&!input.mediaId)throw new Error('Text message body is required');
  if((type==='voice'||type==='face')&&!input.body.trim()&&!input.mediaId)throw new Error('Media message requires a media reference');
  if(input.replyTo!==null&&input.replyTo!==undefined&&typeof input.replyTo!=='string')throw new Error('Invalid reply target');
  if(input.mediaId!==null&&input.mediaId!==undefined&&!/^media_[A-Za-z0-9_-]{10,128}$/.test(input.mediaId))throw new Error('Invalid media ID');
...
function assertConversationInput(input={}){
  ...
  const unique=[...new Set(input.members)];if(unique.length!==input.members.length)throw new Error('Duplicate conversation member');
